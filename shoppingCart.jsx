// simulate getting products from db
const products = [
...
const Products = (props) => {
  ...
  const [{ data, isLoading, isError }, doFetch] = useDataApi(
    "http://localhost:1337/products", //using local Strapi instance
    {
      data: [],
    }
  );
  ...
  let list = items.map((item, index) => {
    ...
    return (
      <li key={index} className="card mb-2 rounded-3 border-4">
        <Image src={uhit} width={70} roundedCircle className="mx-auto"></Image>
        <Button variant="primary" size="large">
          {item.name.replace(/[.,\/#!$%\^&\*;:{}=\-_`~()]/g,"")
} ${item.cost}/ea
        </Button>
        <div className='card-title text-center bg-info'>Stock {item.instock}</div>
  ...
  let cartList = cart.map((item, index) => {
    return (
      <Card key={index}>
        ...
        >
          <Card.Body>
            <p>Click here to remove from cart.</p>
